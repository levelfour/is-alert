...
よりログインし，提出済みにチェックしてください．'''.format((datetime.date.today() + datetime.timedelta(1)).strftime('%m月%d日').decode('utf-8'), homework_list)
        send_mail('[hermite]進捗どうですか？', message, 'Hermite <info@alert.hermite.jp>', [user.email], fail_silently=False)
    ...
    def handle(self, *args, **options):
        for user in User.objects.filter(notification__exact=True):
            ...
            for homework in Homework.objects.filter(deadline__gt=lo, deadline__lt=up):
